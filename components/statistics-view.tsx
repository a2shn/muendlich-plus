...
import { getDB, type Subject, type EvaluationType, type ParticipationEntry, type Grade } from "@/lib/db"
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from "@/components/ui/card"
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs"
import { BarChart3, TrendingUp, Award, Calendar, GraduationCap } from "lucide-react"
...
export function StatisticsView({ subjects, evaluationTypes }: StatisticsViewProps) {
  ...
  const getAllEntriesForSubject = async (subjectId: string): Promise<ParticipationEntry[]> => {
    const db = await getDB()
            ...
            <CardContent>
              {mostActive && mostActive.totalEntries > 0 ? (
                <>
                  ...
                  <p className="text-xs text-muted-foreground mt-1">{mostActive.totalEntries} Meldungen</p>
                </>
              ) : null}
            </CardContent>
          </Card>
                ...
                <div key={subject.id} className="border rounded-lg p-4 space-y-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <h3 className="font-semibold text-lg">{subject.name}</h3>
                      <div className="flex items-center gap-2 text-sm text-muted-foreground">
                        <GraduationCap className="h-4 w-4" />Ø {avgGrade}
                      </div>
                    </div>
                    ...
                    <div className="space-y-3">
                      {correlations.map((corr, idx) => (
                        <div key={idx} className="bg-muted/50 rounded-lg p-3 space-y-2">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                              <div className="text-2xl font-bold">Note: {corr.grade.toFixed(1)}</div>
                          ...
                          <div className="grid grid-cols-2 gap-2 text-sm">
                            {Object.entries(corr.evaluationCounts).map(([evalId, count]) => (
                              <div key={evalId} className="flex items-center gap-2">
                                ...
                                />
                                <span>{getEvaluationTypeName(evalId)}:</span>
